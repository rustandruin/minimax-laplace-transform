...
%Theorem \ref{thm:examplecovarest} states that the eigenvalues of certain random matrices converge, so it is reasonable to expect that one may be able to find such a result using the techniques already developed for handling random matrices. Note, however, the theorem specifies a quantitative rate of convergence, and establishes control over the deviations of the eigenvalues of the empirical covariance matrices both above and below their limits. We now briefly survey the currently available tools, keeping in mind these requirements.


%To illustrate the effectiveness of our approach, we investigate the convergence in relative error of the eigenvalues of the empirical covariance matrix of a Gaussian vector, and the effect of column sampling on the spectrum of a wide matrix with orthonormal rows.
